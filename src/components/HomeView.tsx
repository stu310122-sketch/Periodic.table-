...
export function HomeView() {
    const menuItems = [
        ...
        { title: "數據虛擬實驗室", sub: "科學數據與視覺化模型", icon: "🧊", view: 'visualizations', color: "border-[#ff3366]/30 bg-[#ff3366]/10 hover:bg-[#ff3366]/20 text-[#ff3366]" },
            ...
            <nav className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 w-full max-w-5xl px-4 relative z-10" aria-label="Main Menu">
                {menuItems.map((item, idx) => (
                    <Link 
                        key={idx} 
                        to={`/${item.view}`}
                        className="group relative text-left h-full focus:outline-none focus-visible:ring-4 focus-visible:ring-[#00f0ff] focus-visible:ring-offset-4 focus-visible:ring-offset-black rounded-lg block"
