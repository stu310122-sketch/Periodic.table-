...
export const NetworkBackground: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const nodes: { x: number; y: number; vx: number; vy: number; radius: number; color: string; luminescent: boolean; twinklePhase: number; twinkleSpeed: number }[] = [];
    ...
    const draw = () => {
      ...
      for (let i = 0; i < numNodes; i++) {
        ...
        ctx.globalAlpha = node.luminescent ? Math.max(0.2, Math.abs(Math.sin(time * node.twinkleSpeed + node.twinklePhase))) : 0.8;
